    ...
    def build_model(self):
        ...
        with tf.device("/cpu:0"):
            ...
            self.embedding_w = tf.get_variable(
                "embed_w",
                shape=[self.vocab_size, self.embeddings_dim],
    ...
    def train_for_epoch(self, sess, train_x, train_y):
        ...
        for idx in range(batches_per_epoch):
            batch_idx = np.random.choice(
                train_x.shape[0], size=self.batch_size, replace=False
            )
            batch_xs = train_x[batch_idx, :]
            batch_ys = train_y[batch_idx]
            batch_loss, _, batch_accuracy = sess.run(
                [self.cost, self.train_op, self.accuracy],
                feed_dict={self.x_holder: batch_xs, self.y_holder: batch_ys},
            )
