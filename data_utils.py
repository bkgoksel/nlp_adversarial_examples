...
class IMDBDataset(object):
    def __init__(self, path="aclImdb", max_vocab_size=None):
        ...
        self.train_seqs2 = [
            [w if w < max_vocab_size else max_vocab_size for w in doc]
            for doc in self.train_seqs
        ]

        self.test_seqs = self.tokenizer.texts_to_sequences(self.test_text)
        self.test_seqs2 = [
            [w if w < max_vocab_size else max_vocab_size for w in doc]
            for doc in self.test_seqs
        ]
    ...
    def read_text(self, path):
        ...
        neg_path = path + "/neg"
        pos_files = [
            pos_path + "/" + x for x in os.listdir(pos_path) if x.endswith(".txt")
        ]
        neg_files = [
            neg_path + "/" + x for x in os.listdir(neg_path) if x.endswith(".txt")
        ]
