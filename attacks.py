...
class GeneticAtack(object):
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        new_x_list = [
            self.do_replace(x_cur, pos, w) if x_orig[pos] != w and w != 0 else x_cur
        ...
        if self.use_lm:
            ...
            replace_words_and_orig = [
                self.dataset.inv_dict[w] if w in self.dataset.inv_dict else "UNK"
                for w in replace_list[: self.top_n]
            ] + [orig_word]
    ...
    def perturb(
        self, x_cur, x_orig, neigbhours, neighbours_dist, w_select_probs, target
    ):
        ...
        return self.select_best_replacement(
            rand_idx, x_cur, x_orig, target, replace_list
        )

    def generate_population(
        self, x_orig, neigbhours_list, neighbours_dist, w_select_probs, target, pop_size
    ):
        return [
            self.perturb(
                x_orig, x_orig, neigbhours_list, neighbours_dist, w_select_probs, target
            )
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        tmp = [
            glove_utils.pick_most_similar_words(x_orig[i], self.dist_mat, 50, 0.5)
        ...
        for i in range(x_len):
            if x_adv[i] < 27:
                neighbours_len[
                    i
                ] = 0  # To prevent replacement of words like 'the', 'a', 'of', etc.
        w_select_probs = neighbours_len / np.sum(neighbours_len)
        tmp = [
            glove_utils.pick_most_similar_words(
                x_orig[i], self.dist_mat, self.top_n, 0.5
            )
            for i in range(x_len)
        ]
        neigbhours_list = [x[0] for x in tmp]
        ...
        for i in range(self.max_iters):
            ...
            childs = [
                self.perturb(
                    x, x_orig, neigbhours_list, neighbours_dist, w_select_probs, target
                )
                for x in childs
            ]
            pop = elite + childs
...
class PerturbSentBaseline(object):
    def __init__(
        self, sess, model, batch_model, dataset, dist_mat, lm, n1=20, n2=5, use_lm=True
    ):
        self.sess = sess
        self.model = model
        self.batch_model = batch_model
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        in (x_cur) between the words in replace_list """
        new_x_list = [
            self.do_replace(x_cur, pos, w) if x_orig[pos] != w else x_cur
            for w in replace_list
        ]
