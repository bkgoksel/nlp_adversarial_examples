...
def visualize_attack(sess, model, dataset, x_orig, x_adv):
    ...
    print(
        "Original Prediction = %s. (Confidence = %0.2f) "
        % (
            ("Positive" if np.argmax(orig_pred[0]) == 1 else "Negative"),
            np.max(orig_pred) * 100.0,
        )
    )
    display(HTML(orig_html))
    print("---------  After attack -------------")
    print(
        "New Prediction = %s. (Confidence = %0.2f) "
        % (
            ("Positive" if np.argmax(adv_pred[0]) == 1 else "Negative"),
            np.max(adv_pred) * 100.0,
        )
    )
